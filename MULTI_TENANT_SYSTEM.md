...
**Only TWO live tenants (July 2026).** HRMS + Ticket Management removed completely (frontend deleted, DB rows cascade-deleted via `src/utils/removeLegacyTenants.js`).

| Slug | Name | Dev domain | Prod domain | Status |
|---|---|---|---|---|
| `saisatwik` | SaiSatwik | — (external WordPress) | **`saisatwik.com`** (WordPress/Divi; blogs + analytics wired into Mavro via `wordpressUrl` gate + tracking snippet) | active |
