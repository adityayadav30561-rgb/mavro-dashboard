...
async function fetchWordpressPages(baseUrl) {
  const cacheKey = `wp:${baseUrl}`;
  const hit = cache.get(cacheKey);
  if (hit && hit.expiresAt > Date.now()) return hit.pages;

  const all = [];
  for (let page = 1; page <= 5; page += 1) {
    const url = `${baseUrl.replace(/\/$/, '')}/wp-json/wp/v2/pages?per_page=100&page=${page}&status=publish&_fields=link,title,date`;
  ...
  cache.set(cacheKey, { pages, expiresAt: Date.now() + CACHE_TTL_MS });
  return pages;
}
