...
- **Pages & Blogs are deliverable views, source-aware (`src/services/mbrPagesService.js`):** `/api/mbr/pages` = pages BUILT in the period — WordPress sources (`wordpressUrl` in MBR_SOURCES) pull `/wp-json/wp/v2/pages` publish dates live; registry sources (spanbix) read `SeoMetadata.createdAt` (a new page pushed from code lands in its deploy month via `SPANBIX_STATIC_PAGES`), `/blog*` excluded. `/api/mbr/blogs?source=` — WordPress sources pull `/wp-json/wp/v2/posts` (real publish dates settle GA4 first-traffic mis-attribution; no view counts available); Mavro sources filter the Blog collection to their tenant. WP responses cached 1h. Never GA4-derived.
