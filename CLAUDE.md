...
- **SaiSatwik blogs publish via `npm run create:saisatwik-blog -- <slug> [--publish]`** — data modules in `src/utils/saisatwik-blogs/` (copy `_TEMPLATE.js`), pushed to WordPress over the REST API with an Application Password (`SAISATWIK_WP_URL/USER/APP_PASSWORD` in local `.env` only — Render never needs them). Upserts by slug; draft-first; categories/tags created by name. **LiteSpeed strips the Authorization header** — the `.htaccess` forward rule (documented in BLOG_PUBLISHING.md + the runner header) must exist on the WP install or auth fails with `rest_not_logged_in`. Full process: BLOG_PUBLISHING.md § SaiSatwik.
...
- **MBR is multi-source + exportable.** `MBR_SOURCES` env (JSON array of `{key,label,ga4PropertyId,gscSiteUrl,websiteSlug?,hostname?,credentialsEnv?,wordpressUrl?}`) drives the source tabs on `/mbr` and the per-source sheets in the export; falls back to a single spanbix source from the legacy vars. `credentialsEnv` names another env var holding that source's own service-account key (needed when sources live in different GCP projects — SaiSatwik uses `GOOGLE_SERVICE_ACCOUNT_JSON_SAISATWIK`, property `368831716`, GSC `https://saisatwik.com/`); `googleAuth.js` caches creds + tokens per env var. **Pages view = pages BUILT in the month — never blogs, never traffic** (`/api/mbr/pages`, `src/services/mbrPagesService.js`): `wordpressUrl` sources pull live from the public WP REST API (page publish dates; blog posts are a different WP type, excluded by construction); registry sources (spanbix) read `SeoMetadata.createdAt` — a new page pushed from code must be added to `SPANBIX_STATIC_PAGES` (already required for the sitemap) and lands in its deploy month automatically. Manual workstream rows (PPTs & Videos, Work Log, Other Projects, manual/LinkedIn leads) live in the `MbrItem` collection keyed by `section` + `period` ('YYYY-MM'); section definitions are the single source of truth in `src/config/mbrSections.js` (served via `/api/mbr/sections` — the tiles UI and Excel sheets both render from it; new manual section = one entry there, zero UI code). **"Download MBR"** hits `/api/mbr/export` → styled multi-sheet exceljs workbook (Work Overview index · per-source traffic/search sheets · Blogs with views · manual sheets · Leads Log = website leads auto-merged with manual rows). exceljs is a backend dep — do not import it in `client/`.
