...
- **SaiSatwik WordPress publishing CLI (July 7, night):** `npm run create:saisatwik-blog -- <slug> [--publish]` pushes data modules (`src/utils/saisatwik-blogs/`, `_TEMPLATE.js` with the Spanbix content conventions) to saisatwik.com via the WP REST API + Application Password. Upsert-by-slug, draft-first, categories/tags auto-created, distinct diagnostics per failure mode. **Blocked until user adds the LiteSpeed `.htaccess` Authorization-forward rule** (host strips the header — verified: garbage creds and real creds both return `rest_not_logged_in`). Creds live in local `.env` only (never committed, never on Render). The supplied WP username is still unverified — may need the actual wp-admin login name once the header fix lands. Full doc: BLOG_PUBLISHING.md § SaiSatwik.
