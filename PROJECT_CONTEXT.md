...
⛔ **Mavro Scheduler module — REMOVED July 7, 2026** (user decision: feature unused). The record below is historical only; the code no longer exists (backend module, workers, frontend pages, routes, bullmq/ioredis deps all deleted). Do not rebuild without an explicit new ask.
...
- **Pending (manual, user's accounts):** (1) GA4 → mark `generate_lead` as **Key event** (Recent events → star; appears ~24h after first event; Ads conversion independent of it). (2) Google Ads conversion shows "No recent conversions" until a real ad-driven lead arrives (campaign launched 20 Jun, `Maximise conversions`, ₹600/day, ~₹1,000 funded — flagged to top up / use the ₹20k credit). (3) Free-Render-keep-warm — ✅ RESOLVED: external cron job pings `GET /api/health` every 10 minutes (live, July 2026). Free tier stays awake; cold-start form-submit failures prevented while the cron runs.
...
- **SaiSatwik source live (July 7, later same day):** separate GCP project → per-source `credentialsEnv` support added (`GOOGLE_SERVICE_ACCOUNT_JSON_SAISATWIK`, account `saisatwik-dashboard@saisatwik-analytics.iam.gserviceaccount.com`). GA4 property `368831716`, GSC `https://saisatwik.com/` (apex URL-prefix; hostName check confirmed apex-only traffic + google-translate proxy correctly filtered). Verified live: Jun 590 users (May 692 / Apr 753 — real 3-month history), GSC 318 clicks / 60,735 impressions @ pos 18.5, 15 AI-referral sessions. `MBR_SOURCES` on Render carries both sources. Also fixed: GA4 zero-pads multi-range daily reports across the union of dates — trend series now clamped to their own range bounds.
- **SaiSatwik WordPress publishing CLI (July 7, night):** `npm run create:saisatwik-blog -- <slug> [--publish]` pushes data modules (`src/utils/saisatwik-blogs/`, `_TEMPLATE.js` with the Spanbix content conventions) to saisatwik.com via the WP REST API + Application Password. Upsert-by-slug, draft-first, categories/tags auto-created, distinct diagnostics per failure mode. **⏸ PARKED — awaiting cPanel access.** The LiteSpeed host strips the Authorization header at a level even the WP-generated rewrite rule can't recover (verified: garbage + real creds both `rest_not_logged_in`; Application Passwords ARE enabled per the API index; Wordfence not the blocker). WP File Manager edits kept 403ing (WAF blocks .htaccess writes over HTTP); during attempts the site's .htaccess was briefly lost (inner pages 404) — restored via Settings → Permalinks → Save. **Resume checklist (via cPanel File Manager):** add `SetEnvIf X-WP-Authorization "(.+)" HTTP_AUTHORIZATION=$1` as line 1 of `.htaccess` (SetEnvIf = always-loaded module, zero 500 risk) → run `npm run create:saisatwik-blog -- wp-connection-test` → if `invalid_username`, swap `SAISATWIK_WP_USER` for the real wp-admin login name → once green, revoke + regenerate the Application Password (it was shared in chat). The runner already sends BOTH `Authorization` and `X-WP-Authorization`. Creds in local `.env` only. Full doc: BLOG_PUBLISHING.md § SaiSatwik.
- **Pages & Blogs = deliverable views, source-aware (July 7, evening):** user clarified the MBR "Pages" workstream means pages BUILT that month (not traffic) and blogs must be per-company. New `src/services/mbrPagesService.js` + `/api/mbr/pages`: SaiSatwik pulls live from the public WordPress REST API (`wp/v2/pages` — 89 pages with exact publish dates; posts are a separate WP type so blogs excluded by construction); Spanbix reads the `SeoMetadata` registry (`createdAt` = deploy that introduced the page via `SPANBIX_STATIC_PAGES`) — pushing a new page from code auto-lands it in its deploy month. `/api/mbr/blogs?source=` similarly: WP posts live for SaiSatwik (columns title/link/date, no view counts), tenant-filtered Blog collection for Spanbix (was leaking all tenants). The earlier GA4 first-traffic "new pages" detection was removed (blogs leaked in; the May-published EPPM posts were mis-attributed to June because their traffic started late — WP publish dates settle it). `MBR_SOURCES` gained `wordpressUrl`; hub Blogs tiles link `/mbr/blogs/<key>`. Dashboard cleanup same day: real System Pulse (`/api/analytics/pulse` + per-source GA4/GSC state; hardcoded LIVE list deleted), Publishing Feed + Recent Publications merged into one chronological Activity card with stamps, Device Mix/Top Referrers dropped, Top Pages trimmed to 5.
